  ...
  def index_products(products)
    products.each do |product|
      num = products.index(product) + 1
        puts "#{num}.#{product.name}\t$#{product.price}" 
    end
  end
...
class ClienteView
  ...
  def index(products)
    products.each do |product|
      num = products.index(product) + 1
      puts "#{num}.[ ]#{product.name}, precio: #{product.price}, Numero de productos: #{product.number}" 
    end
  end
  ...
  def products_selled(names, products)
    suma = 0
    products.each do |product|
      num = products.index(product) + 1
      if names.include?(product.name)
        suma += product.price.to_i
        puts "#{num}.[X]#{product.name}" 
      else
        puts "#{num}.[ ]#{product.name}" 
      end
    end
    puts "El total a pagar es: #{suma}"
  end
end
